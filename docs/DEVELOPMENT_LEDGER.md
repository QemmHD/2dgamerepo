...
**Main baseline:** [`d7801e5`](https://github.com/QemmHD/2dgamerepo/commit/d7801e5c14e34690d4027e71e6d114582fc96b43) — documentation reconciliation [#187](https://github.com/QemmHD/2dgamerepo/pull/187) over the First Light feature baseline
**Active branch at grounding:** `agent/first-light-preferences` from `origin/main` at `d7801e5`; A11-10 feature commit [`1cddd9b`](https://github.com/QemmHD/2dgamerepo/commit/1cddd9b) is published in open [PR #188](https://github.com/QemmHD/2dgamerepo/pull/188); PR CI [`29324666979`](https://github.com/QemmHD/2dgamerepo/actions/runs/29324666979) passed, while merge/Pages/live gates remain pending
**Latest shipped feature commit:** [`3ed29e0`](https://github.com/QemmHD/2dgamerepo/commit/3ed29e072b6a5b8d5bd277eb8a3e30de6f6e60f3)
**Shipped ledger baseline before this tranche:** [`d7801e5`](https://github.com/QemmHD/2dgamerepo/commit/d7801e5c14e34690d4027e71e6d114582fc96b43); when this file is on `main`, its containing commit is the newer ledger truth
...
**Full 1.1 remains in flight:** A11-08 and A11-09 are shipped. A11-10 is now in
flight with save-safe Combat HUD size presets, high-contrast combat tells, and seven
source-backed non-color status glyphs. Mono audio, caption detail, independent voice
volume, vibration, hosted delivery, device/AT proof, and A11-14 convergence remain
open. A11-11, A11-13, and A11-14 remain planned; A11-12 remains in flight. A11-05 is
a pulled-forward 1.2 Mines foundation whose remaining full-board proof belongs to the
Fair Forge gate. Selected visual evidence remains local/ignored and does not promote
those rows beyond their earned status.
...
| A11-08 | 1.1 | Validators/CI/harness | `[x] SHIPPED` | Four deterministic gates cover semantic/modality/focus, reduced-motion save inheritance, accessible minigame motion/focus, and phone Settings geometry/action coverage; CI and the real-game harness assert keyboard/reduced Home and Mines receipts. The in-flight A11-10 extension adds a credited combat-cue gate and strict preference receipts. `.github/workflows/ci.yml`, `tools/validate-accessibility.js`, `tools/validate-accessibility-save.js`, `tools/validate-minigame-accessibility.js`, `tools/validate-phone-settings.js`, `tools/validate-combat-cues.js`, `tools/artshot/harness.html` | None | PR #186 shipped the original four gates. Current extension: **19/19 validators**, syntax **152/152**, accessibility **211**, save/preferences **96**, minigame **14**, UX **90**, combat cues **171**, phone Settings **1,579/9**, HUD **5,322/108**, Vigil **67**, and integration **117**. The harness has 13 positive scenarios; Accessibility/130%/contrast/keyboard and gameplay-status states report `EXC:0`, while combined invalid controls report `EXC:4`. Feature `1cddd9b` is published in PR #188; PR CI `29324666979` passed, while merge and deployment remain pending. | Preserve the shipped foundation and land the A11-10 extension without weakening `?dev=1`, invalid-control, or dense-swarm receipts. |
...
| A11-10 | 1.1 | Preference/accessibility suite | `[~] IN FLIGHT` | Current branch implementation: save-safe **Combat HUD size** 100/115/130 with default 100; strict high contrast with default false; desktop/phone Accessibility pane; exact semantic action/announcement; scaled, measured combat HUD lanes; high-contrast HUD, Living Vigil, hazards, telegraphs, and enemy windups; seven source-backed shield/slow/shred/chill/freeze/burn/shock badges with bounded swarm priority. Combat HUD size is not global app/menu text scaling. Preserve separate `?dev=1` Debug Mode, Unlock All Maps (testing), and CHEATS (TESTING). Mono audio, caption detail, independent voice volume, and vibration remain unbuilt. | A11-01 semantics; audio/input setting ownership audit | Local source/logic/runtime proof is green: **19/19 validators**, syntax **152/152**, accessibility **211**, save **96**, UX **90**, combat cues **171**, phone Settings **1,579/9**, HUD **5,322/108**, `EXC:0` menu/gameplay receipts, post-veil/damage-number ordering, and a **540-blit** 180-enemy trash cap. Feature [`1cddd9b`](https://github.com/QemmHD/2dgamerepo/commit/1cddd9b) is published in [PR #188](https://github.com/QemmHD/2dgamerepo/pull/188); PR CI [`29324666979`](https://github.com/QemmHD/2dgamerepo/actions/runs/29324666979) passed. Merge, deploy, and device/AT proof remain pending. | Merge, deploy, and live-smoke this bounded slice. Implement mono, captions, voice volume, and vibration as separately verified follow-ups; do not promote A11-10 or 1.1 yet. |
...
| A11-14 | 1.1 | First Light convergence | `[ ] PLANNED` | Close the exact release gate: five-second comprehension, grayscale/RGB-disabled hierarchy, one-animation attention ceiling, ≤100 ms discrete feedback, pointerless primary loop, five target viewports, text-scale/menu performance fixtures, and paired reduced-motion captures. | A11-01–A11-04 and A11-06–A11-13 complete | Historical PR #186 proof includes accessibility **193**, phone Settings **855**, reduced-frame comparison, and live 667×375/1280×720 smoke; the current A11-10 branch expands local scale/contrast evidence but does not prove convergence. | After the feature rows land, run the named comprehension, timing, viewport, pointerless, grayscale, RGB-disabled, text-scale, and reduced-motion acceptance matrix before calling 1.1 complete. |
...
- [x] Current targeted evidence is recorded: accessibility **211**, save **96**,
      minigame **14**, phone Settings **1,579/9**, UX **90**, combat cues **171**,
      and gambling **644**; the final local Node 22 validator boundary is
      **19/19 OK** with syntax **152/152**.
...
2. **Continue the complete named First Light scope.** A11-01–A11-04, A11-06, and
   ...
   and effective pinch/200% proof. Land A11-10's current scale/contrast/non-color
   slice, then complete mono/captions/voice/vibration; A11-11–A11-13 own the
...
3. **Use the grounded delivery evidence.** The shipped PR #186 baseline and the local
   A11-10 **19/19** validator, syntax **152/152**, accessibility **211**, phone Settings
   **1,579/9**, UX **90**, combat-cue **171**, and HUD **5,322/108** evidence are recorded without
   converting branch-only work into a delivery claim.
...
| 2026-07-14 | A11-10 preference/accessibility tranche / `agent/first-light-preferences` | Marked A11-10 `[~] IN FLIGHT`; added save-safe Combat HUD size 100/115/130, high-contrast post-veil combat tells, seven source-backed non-color status badges, scaled Living Vigil/HUD lanes, desktop/phone Accessibility settings, semantic actions, guided-tour pane routing, and deterministic harness/CI coverage; preserved separate `?dev=1` controls and its profiler diagnostics. Mono audio, caption detail, independent voice volume, and vibration remain unbuilt. | Feature `1cddd9b`; open PR [#188](https://github.com/QemmHD/2dgamerepo/pull/188); PR CI `29324666979` passed. Local gates: **19/19 validators**; syntax **152/152**; accessibility **211**; save **96**; UX **90**; combat cues **171**; phone Settings **1,579/9**; HUD **5,322/108**; Vigil **67**; integration **117**. Real-browser Accessibility/130%/contrast/keyboard and gameplay-status receipts are `EXC:0`; invalid controls are `EXC:4`; dense trash is capped at 540 status blits. Merge and deploy remain pending. | Merge, deploy, and live-smoke this bounded slice; then implement the remaining A11-10 controls separately and keep A11-14/1.1 open. |
