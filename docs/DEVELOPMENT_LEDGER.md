...
**Main baseline:** [`454e944`](https://github.com/QemmHD/2dgamerepo/commit/454e9442ed93dba47a84f469c8e7ed87d3b80f64) — merged [PR #198](https://github.com/QemmHD/2dgamerepo/pull/198), the delivered Collection Growth I-A reachability/source/attachment slice
**Active branch at grounding:** `docs/collection-growth-ia-ledger` from `origin/main` at `454e944`; this branch records the shipped I-A boundary without promoting complete Collection Growth I, its separate 30-look I-B pack, A11-13, full 1.1, the 1.0 → 2.0 arc, or 2.0
**Latest shipped feature commit:** [`454e944`](https://github.com/QemmHD/2dgamerepo/commit/454e9442ed93dba47a84f469c8e7ed87d3b80f64)
**Shipped ledger baseline before this tranche:** [`3f56348`](https://github.com/QemmHD/2dgamerepo/commit/3f563487ac582948719bd28dd3f3c42877677c8c), the [PR #197](https://github.com/QemmHD/2dgamerepo/pull/197) Guided Run Path delivery reconciliation; when this file is on `main`, its containing commit is the newer ledger truth
...
beats/first-death debrief, Daily overhaul, Collection Growth I-B and later collection
depth, House V2,
...
| A11-08 | 1.1 | Validators/CI/harness | `[x] SHIPPED` | Seven A11-focused deterministic gates cover semantic/modality/focus, preference/save inheritance, minigame access, phone Settings, combat cues, gameplay captions, and touch-vibration fallback. The real-game harness asserts cold-boot saved/runtime values, exact transcripts, production boss-warning caption/haptic paths, `?dev=1`, and strict invalid controls; a zero-duration real-Chromium probe checks the production Web Audio graph. PR #196 adds the bounded Run Path validator plus objective draw/text, live-boss retention, and exact 667×375 calibration receipts. PR #198 adds deterministic eight-slot collection reachability/filter/source truth and expands the animated attachment contract through the new styles, Reduced Effects, grant rejection, and menu-route focus transition. `.github/workflows/ci.yml`, `tools/validate-accessibility*.js`, `tools/validate-minigame-accessibility.js`, `tools/validate-phone-settings.js`, `tools/validate-combat-cues.js`, `tools/validate-captions.js`, `tools/validate-haptics.js`, `tools/validate-run-objectives.js`, `tools/validate-cosmetic-collection.js`, `tools/validate-cosmetic-attachments.js`, `tools/artshot/harness.html`, `tools/artshot/audiographprobe.*` | None | PR #190 delivered **21/21 validators** and syntax **157/157**; PR #196 delivered **24/24**, Run Path **93,139**, HUD **14,001/180**, and accessibility **290**. PR #198/main `454e944` raised the integrated boundary to **193,879 assertions**, Collection **8,080**, attachment **5,268** across 162 frames/810 points, progression **5,313**, accessibility **299**, UX **97**, gambling **644**, validators **25/25**, and syntax **168/168**, while preserving Run Path **93,139** and HUD **14,001/180**. PR #190/#196 deployed receipts passed; PR #198 PR/main CI, Pages, and cache-busted deployed HTTP checks passed. | Preserve every shipped gate, especially routed audio, exact transcripts, hidden-surface lifecycle, `?dev=1`, lexical-invalid inputs, 667 px captions/objective rails, active-boss retention, dense swarms, all-73 collection reachability/source truth, case/Boutique exclusions, and attachment-safe motion. |
...
| V1-ECO | Cases/Mines/upgrades | `[~] IN FLIGHT` | LV-04 fair-risk foundation plus PR #198's source-honest Boutique/case inventory separation shipped | Preserve the **644**-check quote/receipt contract, **93%** theoretical Mines return, exact case odds/pity, and the rule that Boutique-only stock never enters cases while case-only stock is never sold directly; implement Forge Reserve as the next economy slice. | LV-04; PR #198; main `454e944` |
| V1-PROG | Chapters/cosmetics/save | `[~] IN FLIGHT` | LV-03 Waylight/progression, PR #196's atomic Run Path settlement, and PR #198's 73-item/nine-set Collection Growth I-A reachability/source slice shipped | Preserve Waylight, Run Path settlement, all-73 source truth, and unchanged save schema. Continue Collection Growth I-B as a separate 30-look pack—the eight I-A looks do not reduce it—then add collection stats, the permanent chapter shelf, and export/recovery. | LV-03; PR #196; PR #198; main `454e944` |
...
| V1-ART | Heroes/bosses/houses | `[~] IN FLIGHT` | PR #192 shipped the six-hero deterministic Blender/animated-attachment foundation; PR #198 ships eight distinct I-A looks/two sets across the shared six-hero × 27-pose rig | Preserve the 18-sheet install manifest, exact head/shoulder/hand pose contract, real silhouette/effect parity, Reduced Effects freeze, and six-hero browser matrix. Continue I-B's separate 30-look silhouette pack—the eight I-A pieces do not reduce it—then produce the 12 boss identity sets and first modular house kit. | PR #192/main `e8ec79f`, historical cosmetic gate **4,387**; PR #198/main `454e944`, current cosmetic gate **5,268** across 162 frames/810 points |
...
- [x] Current integrated evidence is recorded: **193,879 assertions**; Run Path
      **93,139**; HUD **14,001/180**; Collection **8,080**; attachment **5,268**
      across 162 frames/810 points; progression **5,313**; accessibility **299**;
      UX **97**; gambling **644**; validators **25/25**; and syntax **169/169** after
      PR #199 added the PNG receipt validator. PR #198's feature boundary remains
      **168/168**; historical PR #190 subsystem counts remain in their owning rows.
...
- [x] The 73-item/nine-set inventory, eight-item paging, all-73 reachability, exact
      ...
      and the separate unreduced 30-look I-B commitment are explicit.
...
1. **Do not recreate or republish shipped foundations.** LV-01–LV-08 shipped through
   ...
   shipped through PR #198/main `454e944`.
...
3. **Use the grounded delivery evidence.** PR #196/main `5abd6fd` owns the Run Path
   selection/settlement/HUD receipts. PR #198/main `454e944` owns the all-73 Collection
   reachability/source truth, eight-item pages, Boutique stock/set paging, real cosmetic
   silhouettes/effects, six-hero × 27-pose attachments, Reduced Effects freeze, and
   grant/focus-route failures. The current integrated boundary is **193,879 assertions**,
   **25/25** validators, and syntax **169/169** after the receipt validator; PR #198's
   feature boundary was **168/168**. Exact counts are recorded above and in
   `CLAUDE.md`.
4. **Continue Collection Growth I-B as the next bounded content slice.** Preserve
   PR #192's pose/install/browser contract and PR #198's paging, reachability, source,
   attachment, Reduced Effects, and no-power contracts. Do not restore clipped columns,
   bypass source truth, sell case-only stock, place Boutique-only stock in cases, accept
   unknown grants, or alter case odds/pity or Mines return. I-A's eight enabling looks
   do not count toward or reduce I-B's separate 30-look pack; deliver 30 further genuinely
   distinct silhouettes before collection stats/chapter/export work, and do not promote
   the still-incomplete A11-13 lock/context-restoration row.
   Keep A11-11's playable tutorial/first-death debrief moving as release-critical parallel
   work; A11 device/AT proof, House V2, High Refresh, new maps/classes, and full Fair Forge
