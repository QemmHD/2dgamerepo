...
**Main baseline:** [`5abd6fd`](https://github.com/QemmHD/2dgamerepo/commit/5abd6fd1e0c5e06652a244951cb282d973a23f3c) — merged [PR #196](https://github.com/QemmHD/2dgamerepo/pull/196), the delivered deterministic Guided Run Path
**Active branch at grounding:** `docs/guided-run-path-ledger` from `origin/main` at `5abd6fd`; this branch records the shipped guidance boundary without promoting full 1.1, 1.3, the 1.0 → 2.0 arc, or 2.0
**Latest shipped feature commit:** [`5abd6fd`](https://github.com/QemmHD/2dgamerepo/commit/5abd6fd1e0c5e06652a244951cb282d973a23f3c)
**Shipped ledger baseline before this tranche:** [`0787254`](https://github.com/QemmHD/2dgamerepo/commit/0787254156d1f82cd4c0147d1d64091ca8b52b2f), the [PR #195](https://github.com/QemmHD/2dgamerepo/pull/195) exact campaign-gate reconciliation; when this file is on `main`, its containing commit is the newer ledger truth
...
beats/first-death debrief, Daily overhaul, collection growth, House V2,
...
| A11-08 | 1.1 | Validators/CI/harness | `[x] SHIPPED` | Seven A11-focused deterministic gates cover semantic/modality/focus, preference/save inheritance, minigame access, phone Settings, combat cues, gameplay captions, and touch-vibration fallback. The real-game harness asserts cold-boot saved/runtime values, exact transcripts, production boss-warning caption/haptic paths, `?dev=1`, and strict invalid controls; a zero-duration real-Chromium probe checks the production Web Audio graph. PR #196 adds the bounded Run Path validator plus objective draw/text, live-boss retention, and exact 667×375 calibration receipts. `.github/workflows/ci.yml`, `tools/validate-accessibility*.js`, `tools/validate-minigame-accessibility.js`, `tools/validate-phone-settings.js`, `tools/validate-combat-cues.js`, `tools/validate-captions.js`, `tools/validate-haptics.js`, `tools/validate-run-objectives.js`, `tools/artshot/harness.html`, `tools/artshot/audiographprobe.*` | None | PR #190 delivered **21/21 validators**, syntax **157/157**, accessibility **288**, save/preferences **125**, captions **29**, haptics **21**, minigame **14**, UX **90**, combat cues **171**, phone Settings **2,430/9**, HUD **8,294/144** including 667 px landscape, Vigil **67**, and integration **117**. PR #196 then passed **24/24** validators, Run Path **93,139**, HUD **14,001/180**, and accessibility **290**, including representative desktop/tablet/phone/boss Chromium states. Both releases' PR/main CI, Pages, and deployed receipts passed. | Preserve every shipped gate, especially the routed-audio graph, exact transcript, hidden-surface lifecycle, `?dev=1`, lexical-invalid, 667 px captions/objective rails, active-boss retention, and dense-swarm receipts. |
...
| V1-PROG | Chapters/cosmetics/save | `[~] IN FLIGHT` | LV-03 Waylight/progression plus PR #196's held-coin atomic receipts and terminal completed-phase Deeds-XP derivation shipped | Preserve Waylight and Guided Run Path settlement; deliver Collection Growth I through the shipped pose rig, then build collection stats, the permanent chapter shelf, and export/recovery. | LV-03; PR #196; main `5abd6fd` |
...
| V1-ART | Heroes/bosses/houses | `[~] IN FLIGHT` | PR #192 shipped the six-hero deterministic Blender/animated-attachment foundation; gameplay silhouettes/collision measured | Preserve the 18-sheet install manifest, exact head/shoulder/hand pose contract, and six-hero browser matrix; deliver Collection Growth I toward the first 30 genuinely distinct silhouettes, then produce the 12 boss identity sets and first modular house kit. | PR #192; main `e8ec79f`; cosmetic gate **4,387** |
...
- [x] Current release evidence is recorded: Run Path **93,139**, HUD **14,001/180**,
      accessibility **290**, validators **24/24**, and syntax **166/166**. Historical
      PR #190 subsystem counts—save **125**, captions **29**, haptics **21**, minigame
      **14**, phone Settings **2,430/9**, UX **90**, combat cues **171**, and gambling
      **644**—remain recorded in their owning rows.
...
1. **Do not recreate or republish shipped foundations.** LV-01–LV-08 shipped through
   ...
   the Guided Run Path shipped through PR #196/main `5abd6fd`.
...
3. **Use the grounded delivery evidence.** PR #196/main `5abd6fd` passed its six-mode
   director selection/completion matrix, mode-agnostic settlement fixtures, all-26
   static next-action wide-font gate, seven representative real-Chromium states,
   667×375 field/live-boss rails, compressed desktop/tablet layouts, strict invalid
   controls, main CI, Pages, deployed Run Path, and five-control `?dev=1` smoke. Exact
   counts and invariants are recorded above and in `CLAUDE.md`.
4. **Begin Collection Growth I as the next bounded content slice.** Preserve PR #192's
   animated pose/install/browser contract and add the first genuinely distinct looks
   toward the 30-look silhouette pack; palette-only duplicates do not count. Ship source
   labels, compatible anchors, preview/live parity, paging/filter fixtures, and no power.
