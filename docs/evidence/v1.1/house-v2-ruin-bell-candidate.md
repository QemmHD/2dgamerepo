...
Status: **implemented, committed as `04e1c96`, published in draft PR #205, and locally
validated at 36/36 validators plus syntax/YAML/diff gates, but not yet accepted by
hosted CI/browser review, merged, deployed, or shipped**.

This record describes the bounded candidate on
`feature/house-v2-ruin-bell`, based on `origin/main` at
`3449a18cf4ada4eac0926f0259275fae558f622d`. The exact code candidate is
[`04e1c963d9a3af040edd0624c25c9ecfa710b837`](https://github.com/QemmHD/2dgamerepo/commit/04e1c963d9a3af040edd0624c25c9ecfa710b837)
...
The player-facing right rail remains a single guidance owner. Ruin Bell state replaces
the Run Path card only while eligible/active, reports attempt, action, progress,
countdown, and exact reward, and supplies a dedicated accessibility string. High
Contrast and Reduced Effects retain static doorway, role, timer, and objective truth.
The five `?dev=1` Settings controls are intended to remain unchanged.
...
House V2 multiplication gate. `tools/validate-navigation.js` currently passes **73,773
checks**, covering **176** legacy approaches plus **62** House V2 exit/formation routes
and an 180-body stress fixture. `tools/validate-house-v2.js` currently passes **901
...
The local workflow changes define eight production-harness states, but no hosted run or
accepted artifact exists yet:

| Required receipt | Canvas and required accessibility state |
| --- | --- |
...
| `ruin-bell-crossfire-667x375.png` | Exact 667x375, touch, 130% HUD, Reduced Effects |
...
accessible objective owner, zero direct coins, non-empty PNG, exact backing dimensions,
and `data-qa-ready`/zero-exception completion. Success additionally requires exactly
...
eight non-empty PNGs with eight unique SHA-256 hashes. A human must then open every
...
## Local validation snapshot - 2026-07-15

These commands were run locally against the code now identified by candidate commit
`04e1c96`; they are immutable code identity but not hosted or release evidence:

| Command | Observed result |
| --- | --- |
| `node tools/validate-house-v2.js` | PASS - 901 checks; 180-body stress 101,937 probes |
| `node tools/validate-navigation.js` | PASS - 73,773 checks; 176 legacy + 62 House V2 routes; 180-body stress 86,488 probes |
...
| `node tools/validate-ruin-bell.js` | PASS - 725 checks; exact encounter, defense, retry, placement, reward claim provenance, no-stale-card, large-step terminal spawn-lane closure, runtime/UI/render/audio/dev seams |
| Full local repository gate | PASS - 36/36 validators, full Node syntax, harness-module syntax, CI YAML parse, and diff check; zero reported failures |

All six named targeted commands and the complete local repository gate pass in the
current mutable working tree. That is still not hosted or release validation. The
hosted Chromium/Web Audio matrix, eight-PNG artifact, original-resolution visual
review, long-run performance/audio soak, and physical-device/assistive-technology
checks have not been reconciled for this branch.
