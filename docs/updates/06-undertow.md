...
- **Unlock gate**: `stats.totalBosses >= 6` (tunable) — the same lifetime counter map unlocks use (`src/content/maps.js:129-133`, accumulated at `src/systems/SaveSystem.js:466`). Locked state renders the CTA dimmed with "SLAY 6 KEEPERS" subtext, matching the map-unlock language.
...
**Work:**
- Add 'startDescent' action + CTA (MenuRenderer.js:1139-1177 pattern) with totalBosses>=6 gate
