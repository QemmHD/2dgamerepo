...
function defaultData() {
    return {
        ...
        dailyRoad: { day: 0, best: 0, prevBest: 0 },
...
export class SaveSystem {
    ...
    _validate(data) {
        ...
        // Old saves lack the field(s) → defaults (implicit migration, no bump).
    ...
    // record never masquerades as "yesterday"). Returns
    // { best: new-best-today?, firstToday: first daily score of this day? }
    // — firstToday drives the once-a-day free-case payout (Game._bankDailyRoad).
    recordDailyRoadScore(day, score) {
        const old = this.data.dailyRoad;
        const firstToday = !old || typeof old !== 'object' || old.day !== day;
        if (firstToday) {
            this.data.dailyRoad = { day, best: 0, prevBest: (old && old.day === day - 1) ? (old.best ?? 0) : 0 };
        }
        const v = Math.max(0, Math.floor(score || 0));
        let best = false;
