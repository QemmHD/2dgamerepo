...
| `src/assets/hero/monkey_{down,up,side}.png` | Generated | higgsfield (Nano Banana 2) | THE HERO BODY — HQ-pixel upgrade of the shared chibi wick-keeper monkey, 7 frames per direction (idle×2 incl. blink, walk×3, cast, hurt), generated as 2×2 pose grids img2img from the procedural body as reference (same silhouette/anchors), packed by `tools/artshot/hero-pack.mjs`. ALL six heroes render from this base via palette tint + code-drawn feature overlays (`HeroAiSprites.js`), so cosmetics/weapon-arm keep fitting; the procedural body remains the fallback. |
